...
public class World {
    ...
    public World() {
        ...
        for(int i = 0; i < 7; i++){
            for(int j = 0; j < 7-i; j++){
                tiles[i][j].setThing(new Tree(this));//左上角的树
            }
        }
        for(int j = 0; j < 8; j++){
            for(int k = 0; k < 5; k++){
                int i = 7-j+3*k;
                tiles[i][j].setThing(new Water(this));//左上角的河和草
        ...
        for(int i = 0; i < 13; i++){
            tiles[i][8].setThing(new Water(this));//河
        }

        for(int i = 0; i < 4; i++){
            for(int j = 10; j < 14; j++){
                tiles[i][j].setThing(new House(this));//小木屋
            }
        }

        int tempj = 9, tempi = 12;
        while(tempj < HEIGHT){
            tiles[tempi][tempj].setThing(new Water(this));//完整的河
        ...
        for(int j = 15; j < HEIGHT; j++){
            for(int i = 0; i < WIDTH; i++){
                ...
                if((i == 23 && j == 28) || (i == 7 && j == 25) || (i == 22 && j ==36) || (i == 16 && j == 40))
                    ...
                    tiles[i][j].setThing(new Tree(this));//河左侧的随机树
                else if(!flag && j > 40 && Math.random() < 0.05){
                    tiles[i][j].setThing(new Boat(this));//船
                    flag = true;
                }
                else if(num < 3 && j > 30 && Math.random() < 0.01){
                    tiles[i][j].setThing(new EnergyFood(this));//生命值
                    num += 1;
                }
            }
        }
